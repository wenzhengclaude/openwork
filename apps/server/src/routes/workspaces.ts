  ...
  try {
    ...
    if (!response.ok) {
      throw new ApiError(
        ...
        `Open One workspace discovery failed (${response.status} ${response.statusText || "HTTP error"})`,
      );
    }
    return await response.json();
  } catch (error) {
    if (error instanceof ApiError) throw error;
    throw new ApiError(502, "openwork_workspace_discovery_failed", "Open One workspace discovery failed", {
      error: String(error),
    });
  } finally {
...
export function registerWorkspaceRoutes(options: RegisterWorkspaceRoutesOptions): void {
  ...
  addRoute(routes, "DELETE", "/workspaces/:id", "host", async (ctx) => {
    ...
    await recordAudit(workspace.path, {
      ...
      target: "workspace",
      summary: "Deleted workspace from Open One server",
      timestamp: Date.now(),
    });
