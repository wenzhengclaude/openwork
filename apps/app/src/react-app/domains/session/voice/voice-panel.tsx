...
const DEFAULT_TEXT_COMMAND = "Summarize the current Open One session and put the next step in the composer.";
...
const TOOL_LABELS: Record<string, string> = {
  openwork_snapshot: "Checking Open One",
...
function humanToolLabel(toolName?: string) {
  if (!toolName) return "Open One action";
...
async function executeOpenWorkTool(name: string, args: Record<string, unknown>) {
  ...
  if (!control) return { ok: false, error: "Open One control surface is not available." };
  ...
  return { ok: false, error: `Unknown Open One voice tool: ${name}` };
}
...
export function VoicePanel(props: VoicePanelProps) {
    ...
    setVoiceRuntimeSnapshot((current) => ({
      ...
      statusText: text ?? (
        nextStatus === "connecting" ? "Connecting to OpenAI Realtime..." :
          nextStatus === "listening" ? "Listening. Ask Open One to act." :
            nextStatus === "speaking" ? "Open One is speaking..." :
  ...
  const connectRealtime = useCallback(async (audioInput = true) => {
    ...
    if (!client) throw new Error("Open One host connection is not ready.");
    ...
    if (audioInput) {
      ...
      if (!macPermissionGranted) throw new Error("macOS denied microphone access. Enable Open One in System Settings > Privacy & Security > Microphone, then restart Open One.");
    ...
    addEntry("system", `Realtime connected with ${realtimeSession.model} and ${realtimeSession.tools.length} Open One tools.`);
    ...
    <div ref={panelRef} className="flex h-full min-h-0 flex-col bg-background">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-muted-foreground">Realtime voice over Open One UI MCP controls</div>
        </div>
          ...
          {!props.client ? (
            <Card variant="outline" size="sm">
              ...
              <CardContent className="text-sm text-muted-foreground">
                Voice Mode needs the local Open One server so it can mint short-lived Realtime client secrets without exposing your API key to the renderer.
              </CardContent>
            </Card>
          ) : null}
