/**
 * Shared "you can return to Open One now" HTML page for OAuth callbacks.
...
export function connectCallbackPage(input:
  ...
  const openWorkUrl = "openone://settings/extensions"
  const body = input.ok
    ? `<p>${escapeHtml(input.name)} is connected. You can return to Open One now.</p>
      <p><a href="${openWorkUrl}" style="display:inline-block; margin-top:16px; border-radius:10px; background:#0f172a; color:white; padding:10px 14px; text-decoration:none; font-weight:600;">Open Open One</a></p>
  ...
  return `<!doctype html>
<html>
  <head><meta charset="utf-8"><title>${title} - Open One</title></head>
