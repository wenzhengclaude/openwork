...
const FLOW_ID = "grok-selected-skill-agent-run";
...
async function waitForRun(workspaceId, runId, tokens) {
  ...
  throw new Error(`Timed out waiting for Grok selected-skill run ${runId}. Last status: ${latest?.run?.status ?? "unknown"}`);
}
...
export default {
  id: FLOW_ID,
  title: "Grok Build consumes the selected Open One skill without broad skill scanning",
  kind: "internal",
  requiresApp: false,
  steps: [
    {
      name: "Selected sap-login skill reaches Grok through Open One",
      run: async (ctx) => {
        await ctx.prove("A Grok Build agent run receives sap-login as structured selected skill context", {
          assert: async () => {
            ...
            ctx.assert(run.status === "completed", `Grok selected-skill run completed (actual: ${run.status})`);
            ctx.assert(selectedSkillLoad.includes("sap-login"), "The selected sap-login skill was loaded by the runtime.");
            ctx.assert(!fullPromptLeak, "The internal collaboration prompt was not emitted as user-visible Grok output.");
            ctx.assert(failedEvents.length === 0, `No failed/error agent-run events were emitted (actual: ${failedEvents.length})`);
            ctx.output("Grok selected skill run", [
